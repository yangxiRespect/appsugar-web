...
public abstract class BaseRedisTest extends SpringBaseTest {
	public static final Logger log = LoggerFactory.getLogger(BaseRedisTest.class);
	...
	private static void loadDBOnce(ApplicationContext ctx) {
		...
		synchronized (BaseRedisTest.class) {
			if (loadedDB) {
				return;
			}
			loadedDB = true;
